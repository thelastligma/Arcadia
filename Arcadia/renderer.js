...
async function refreshStatus() {
  try {
    ...
    const opiumPort = window.electron?.checkOpiumware ? await window.electron.checkOpiumware() : null;
...
function updateExecutorPortBar() {
  ...
  if (executor === 'macsploit') {
    ...
    barExecutor.textContent = 'Opiumware';
  } else {
...
async function sendToServer() {
  ...
  try {
    ...
    if (executor === 'hydrogen') {
      ...
      showToast('Searching for Opiumware local servers...', 3000);
      try {
        const result = await window.electron.sendToOpiumware(script, 'ALL');
        showToast('Script sent to Opiumware successfully.', 4000);
        console.log('Opiumware response:', result);
      } catch (err) {
        console.error('Opiumware send failed:', err);
        const firstPort = OPIUMWARE_PORTS[0];
        const lastPort = OPIUMWARE_PORTS[OPIUMWARE_PORTS.length - 1];
        showToast('Could not reach Opiumware on ports ' + firstPort + '-' + lastPort, 5000);
      }
    } else {
