...
async function executeOpiumware(scriptContent, port = 'ALL') {
  const ports = ['8392', '8393', '8394', '8395', '8396', '8397'];
  const targets = port === 'ALL' ? ports : [String(port)];
  ...
  if (!stream) {
    throw new Error('Failed to connect on all ports');
  }

  if (scriptContent && scriptContent !== 'NULL') {
    const payload = (() => {
      const trimmed = String(scriptContent).trim();
      if (trimmed.startsWith('OpiumwareScript') || trimmed.startsWith('OpiumwareSetting')) {
        return trimmed;
      }
      return `OpiumwareScript ${trimmed}`;
    })();

    await new Promise((resolve, reject) => {
      zlib.deflate(Buffer.from(payload, 'utf8'), (err, compressed) => {
...
function createWindow() {
  ...
  // Send to Opiumware using its TCP protocol (ports 8392-8397)
  ipcMain.handle('send-to-opiumware', async (event, scriptContent, port = 'ALL') => {
    try {
      return await executeOpiumware(scriptContent, port);
    } catch (err) {
      console.error('Failed to send to Opiumware:', err);
      throw err;
    }
  });
  ...
  // Health check for Opiumware: attempt TCP connect to 8392-8397
  ipcMain.handle('check-opiumware', async () => {
    const ports = ['8392', '8393', '8394', '8395', '8396', '8397'];
    for (const p of ports) {
      try {
        const ok = await new Promise((resolve) => {
          const sock = net.createConnection({ host: '127.0.0.1', port: Number(p) }, () => {
            sock.destroy();
            resolve(true);
          });
